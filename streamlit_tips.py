...
st.write(
    """Your project needs visuals, whether that\'s a video, audio clip, image, map, chart, or table. Streamlit supports
...
st.write(
    """Any variable can be made interactive by using a widget. Use widgets like `st.slider`, `st.selectbox`, and `st.radio`
...
st.write(
    """By adding `st.sidebar` to any of your widgets you can move them to the sidebar on the left. This
cleans up the layout and makes sure that your controls are always visible. 

_Note: some widgets are best
...
st.write("#### 👈👈 &nbsp; use the selectbox to change the font weight of the header!")

"-------------------"

st.write(subheader_size, "Tip 4: Add some text")
st.write(
    """Write out the important parts that need explanation or use text to break up the layout of the app.
...
st.write(subheader_size, "Tip 5: Hide supporting information")
st.write(
    """To keep your app layout clean, you can add in explanations or extra info, graphs, etc. by hiding
...
st.write(
    """You want the app to run quickly for your users, so use `st.cache` for any expensive
computation or data loads. Read more about [how st.cache works](https://docs.streamlit.io/caching.html)
...
st.write(
    """If your project is a step by step walk through - then ignore this. If it\'s an app for someone to use, then you want
...
st.write(
    """One way you can do that is with `st.empty`, which allows you to choose the order of where elements appear in the app.
...
st.write(
    """If you're teaching a concept it can be useful to show off some code. You can use `st.code` to format code, or if
...
st.write(
    """Whenever possible try to use more exciting or fun examples, or for bonus points, add an
